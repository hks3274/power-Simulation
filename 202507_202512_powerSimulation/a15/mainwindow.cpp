    ...
    , m_data(dataMng)
{
    ...
    hWidget    = new harmonicwidget(m_data);
    ...
    rightTopDock = new QDockWidget("고조파 생성창", this);
    rightTopDock->setWidget(hWidget);
    ...
    //주파수 추적시 cps SpinBox 입력값 변경
    connect(tracker, &frequencytracker::cpsChanged,
            iWidget,   &inputWidget::setCpsFromTracker);
