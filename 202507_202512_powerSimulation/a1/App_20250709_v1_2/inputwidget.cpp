    ...
    : dataMng(dataMng) // 멤버 초기화 리스트,  클래스 내부에 있는 멤버 변수 dataMng를 매개변수 dataMng로 초기화
{
...

    {
        ...
        auto initPhase = dataMng.getPhaseValue();
        phaseVal = new QDial();

        phaseVal->setValue(initPhase);
        phaseVal->setRange(0,360);
        phaseVal->setStyleSheet(R"()");
        hl->addWidget(name);
        hl->addWidget(dataVal);
        hl->addWidget(sign);
        hl->addWidget(phaseLabel);
        hl->addWidget(phaseVal);
        ...
        connect(phaseVal, &QDial::valueChanged, this, &inputWidget::phaseValueChanged);
    }
...
void inputWidget::phaseValueChanged() {
    float val = static_cast<int>(phaseVal->value());

    qDebug() << "위상 : " << val;
    dataMng.setPhaseValue(val);
}
