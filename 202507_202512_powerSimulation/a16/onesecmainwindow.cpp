...
void oneSecMainWindow::onRmsData(dataManagement::measure_data m)
{
    ...
    auto s = dataMng.getLatestVoltageAndCurrent(); // wave form 값 가져오기
    oneSecSampleCnt++;
    ...
    //Residual 계산
    // 잔류 전압 계산 (순간값)
    double residualV = s.vAVal + s.vBVal + s.vCVal;
    double residualI = s.cAVal + s.cBVal + s.cCVal;
    ...
    if (elapsed >= std::chrono::milliseconds(995)) {
        ...
        for (int r = 0; r < 4; ++r) {
            ...
            for (int c = 0; c < 3; ++c) {
                ...
                tableFund->setItem(r, c + 1,
                                   new QTableWidgetItem(QString::number(fundVals[r][c], 'f', 2) + unit));
            }
        }
        ...
        tableHarm->setItem(1, 1, new QTableWidgetItem(QString::number(vHph, 'f', 2) + "°"));
        tableHarm->setItem(1, 2, new QTableWidgetItem(QString::number(iHph, 'f', 2) + "°"));
        ...
        // === Residual RMS 계산 ===
        double vResidualRMS = std::sqrt(sumResidualV / oneSecSampleCnt);
        double iResidualRMS = std::sqrt(sumResidualI / oneSecSampleCnt);
        ...
        //==========simmentic component==================
        std::complex<double> a(-0.5, std::sqrt(3)/2.0);
        std::complex<double> a2 = std::pow(a, 2);
