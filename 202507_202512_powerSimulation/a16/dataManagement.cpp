...
void dataManagement::captureData()
{
    ...
    if (vHarmonicOrder > 1) {
        ...
        const double vHarmCommon = vHarmonicMag * std::sin(vh * base + vHextra);
...
dataManagement::phasor_data dataManagement::calcPhasor() const {
    ...
    if (std::abs(hvX) < 1e-8) // 고조차 크기가 0일떄 노이즈제거
        hvX = std::complex<double>(0,0);
    if (std::abs(hcX) < 1e-8) //고조파 크기가 0일떄 노이즈 제거
        hcX = std::complex<double>(0,0);
