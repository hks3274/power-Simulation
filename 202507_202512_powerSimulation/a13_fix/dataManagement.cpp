    ...
    if(N > 0 && sampleCnt >= N) { // 한 주기
        ...
        if (!voltage_data_queue.empty()) {
            ...
            for (int n = 0; n < N; ++n) {
                ...
                vX += voltage_data_queue[start + n].value * W1;
            }
            vX /= (double)N;
            ...
            for (int n = 1; n < N; ++n) {
                ...
                double t0 = std::chrono::duration<double>(voltage_data_queue[start + n - 1].t).count();
                double t1 = std::chrono::duration<double>(voltage_data_queue[start + n].t).count();
