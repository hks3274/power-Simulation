...
void phasorWidget::refreshLegend()
{
    auto ph = dataMng.calcPhasor();
    // 기본파
    QString vTxt = QString("기본 V %1, %2°")
                       .arg(ph.vMag, 0, 'f', 2)
                       .arg(ph.vPhaseDeg, 0, 'f', 0);
    QString iTxt = QString("기본 I %1, %2°")
                       .arg(ph.cMag, 0, 'f', 2)
                       .arg(ph.cPhaseDeg, 0, 'f', 0);

    // 고조파
    QString vhTxt = QString("고조파 V h=%1, %2, %3°")
                        .arg(ph.vOrderUsed)                       // calcPhasorHarmonic에서 채움
                        .arg(ph.hvMag, 0, 'f', 2)
                        .arg(ph.hvPhaseDeg, 0, 'f', 0);
    QString ihTxt = QString("고조파 I h=%1, %2, %3°")
                        .arg(ph.cOrderUsed)
                        .arg(ph.hcMag, 0, 'f', 2)
    ...
    // 폰트 크기 줄이기
    QFont smallFont = cbV->font();
    smallFont.setPointSize(8);   // 기존보다 조금 작게
    cbV->setFont(smallFont);
    cbI->setFont(smallFont);
    cbVH->setFont(smallFont);
    cbIH->setFont(smallFont);
