...
//qsutomplot에 맞춘 데이터 가공처리 std::deque<data> -> QVector<bouble> 형태로 변경,
//why? qcustomplot의 setData(QVector<bouble>, QVector<double>) 형태로 들어가기 때문
QPair<QVector<double>, QVector<double>> dataManagement::getPlotDataInRangeSec(double rangeSec)
{
    QVector<double> xData, yData;
    if (data_queue.empty()) return {xData, yData};

    double maxTime = duration<double>(data_queue.back().t).count();
    double minTime = maxTime - rangeSec;

    for (const auto& pt : data_queue) {
        double t = duration<double>(pt.t).count();
        if (t >= minTime) {
            xData.append(t);
            yData.append(pt.value);
        }
    }
    return {xData, yData};
}
