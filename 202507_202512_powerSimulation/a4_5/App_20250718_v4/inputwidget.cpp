    ...
    : dataMng(dataMng) // 멤버 초기화 리스트,  클래스 내부에 있는 멤버 변수 dataMng를 매개변수 dataMng로 초기화
{
    ...
    //sampling 시간 간격 입력
    {
        ...
        samplingCycleSpinBox->setMaximum(20);
...

    {
        ...
        cycleSampleNumSpinBox->setMaximum(20);
        cycleSampleNumSpinBox->setMinimum(1);
        cycleSampleNumSpinBox->setValue(1);
...

    {
        ...
        QLabel *timeIntervalLabel = new QLabel( " 시간간격 ( 1 / (A*B) * 1000 ) : " );

        timeInterVal = new QLabel(QString::number(dataMng.getTimeInterval())+"ms");
...
{
    float interval_ms = (1.0f / (samplingCycleSpinBox->value() * cycleSampleNumSpinBox->value())) * 1000.0f;
    timeInterVal->setText(QString::number(interval_ms) +"ms");

    qDebug() << "시간간격 : " << interval_ms;
