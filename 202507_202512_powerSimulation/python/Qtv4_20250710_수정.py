...
from PyQt6.QtGui import QFont, QIntValidator, QDoubleValidator, QPainter
...
class MyApp(QMainWindow):
    ...
    def initUI(self):
        ...
        self.iSlider.setMinimum(-500)
        self.iSlider.setMaximum(500)
        ...
        # === 벡터 다이어그램 추가 ===
        self.phasor_widget = PhasorWidget(graph_widget) #그래프 위젯(Sub2 창)에 PhasorWidget 생성
        self.phasor_widget.setFixedSize(300, 300) #사이즈 지정
        self.phasor_widget.move(50, 800) # 위치 지정
    ...
    def update_graph(self):
        ...
        # 전압 각도 계산
        v_angle = 2 * math.pi * self.frequency * self.time + math.radians(self.vDial.value())

        # 전류 각도 계산
        i_angle = 2 * math.pi * self.frequency * self.time + math.radians(self.iDial.value())

        # 벡터 위젯에 각도 전달
        self.phasor_widget.update_angles(v_angle, i_angle)
...
class PhasorWidget(QWidget):
    def __init__(self, parent=None):
        ...
        self.voltage_angle = 0
        self.current_angle = 0
        self.radius = 100

    def update_angles(self, voltage_angle, current_angle):
    ...
    def paintEvent(self, event):
        ...
        cx = self.width() // 2 #원의 x중심
        cy = self.height() // 2 #원의 y중심

        # 원 그리기
        painter.setPen(QPen(Qt.GlobalColor.gray, 1))
        # void QPainter::drawEllipse(int x, int y, int width, int height) =>(x,y) 위치 부터 width, height 크기의 원을 그림
        painter.drawEllipse(cx - self.radius, cy - self.radius, self.radius * 2, self.radius * 2)

        # 전압 벡터 (빨강)
        vx = cx + self.radius * math.cos(self.voltage_angle)
        vy = cy - self.radius * math.sin(self.voltage_angle)
        painter.setPen(QPen(Qt.GlobalColor.red, 3))
        painter.drawLine(cx, cy, int(vx), int(vy))
        painter.drawText(int(vx), int(vy), "V")

        # 전류 벡터 (파랑)
        ix = cx + self.radius * math.cos(self.current_angle)
        iy = cy - self.radius * math.sin(self.current_angle)
        painter.setPen(QPen(Qt.GlobalColor.blue, 3))
        painter.drawLine(cx, cy, int(ix), int(iy))
        painter.drawText(int(ix), int(iy), "I")
