...
void dataManagement::captureData()
{
    ...
    const double fundAC = iAValue * std::sin(base + iA_abs);   // φ 더하지 않음 (절대위상)
    ...
    const double vHextra = 2.0 * M_PI * (double)vHarmonicPhase / 360.0; // 전압 고조파 전역 오프셋
    const double cHextra = 2.0 * M_PI * (double)cHarmonicPhase / 360.0; // 전류 고조파 전역 오프셋

    // 전압 고조파: h*phiX + 전역 오프셋
    const double vHarmA = vHarmonicMag * std::sin(vh*base + vh*phiA + vHextra);
    const double vHarmB = vHarmonicMag * std::sin(vh*base + vh*phiB + vHextra);
    const double vHarmC = vHarmonicMag * std::sin(vh*base + vh*phiC + vHextra);

    // 전류 고조파: h*phiX(상 회전) + 전역 오프셋 (절대위상 체계)
    const double cHarmA = cHarmonicMag * std::sin(ch*base + ch*phiA + cHextra);
    const double cHarmB = cHarmonicMag * std::sin(ch*base + ch*phiB + cHextra);
    const double cHarmC = cHarmonicMag * std::sin(ch*base + ch*phiC + cHextra);
    ...
    // 큐에 삽입
    data_queue.push_back({ vAVal, cAVal, vBVal, cBVal, vCVal, cCVal, simulationTime });
    ...
    harmSumBV += vHarmA   * vHarmA;
    harmSumBC += cHarmA   * cHarmA;
    sumBV     += vBVal  * vBVal;
    sumBI     += cBVal  * cBVal;
    sumP_B    += vBVal  * cBVal;
    ...
    if (N > 0 && sampleCnt >= N) { // 한 주기(또는 그에 준하는 창)마다
        ...
        if (!data_queue.empty()) {
            const size_t vSz   = data_queue.size();
            const size_t start = (vSz >= static_cast<size_t>(N)) ? (vSz - N) : 0;

            // --- 1. 기본파 성분 추출 (DFT k=1) ---
            std::complex<double> vX(0,0);
            for (int n = 0; n < N; n++) {
                const double th1 = -2.0 * M_PI * n / N;  // k=1
                std::complex<double> W1(std::cos(th1), std::sin(th1));
                vX += data_queue[start + n].vAVal * W1;
            }
            vX /= (double)N;

            // sin 기준 보정 (cos→sin 변환)
            std::complex<double> vZ(-vX.imag(), vX.real());

            // 기본파 크기 (peak)와 위상(rad)
            double A    = std::hypot(vZ.real(), vZ.imag()) * std::sqrt(2.0);
            double phi  = std::atan2(vZ.imag(), vZ.real());

            // --- 2. 기본파 파형을 재구성해서 Zero Crossing 찾기 ---
            for (int n = 1; n < N; n++) {
                // 샘플 타임스탬프 (큐에 저장된 실제 시간 이용)
                double t0 = std::chrono::duration<double>(data_queue[start + n - 1].t).count();
                double t1 = std::chrono::duration<double>(data_queue[start + n].t).count();

                // 기본파 재구성
                double v0 = A * std::sin(2*M_PI*frequency*t0 + phi);
                double v1 = A * std::sin(2*M_PI*frequency*t1 + phi);

                if (v0 < 0.0 && v1 >= 0.0) {
                    // 보간으로 Zero Crossing 시각 추정
                    const double dv = v1 - v0;
                    double tZC = t1;
                    if (std::fabs(dv) > 1e-12) {
                        const double frac = (-v0) / dv;
                        tZC = t0 + std::clamp(frac, 0.0, 1.0) * (t1 - t0);
                    }
                    lastZcSec = tZC; // 기본파 기준 ZC 기록
                    break;           // 한 번만 찾으면 끝
                }
            }
        }
    ...
    simulationTime += duration_cast<nanoseconds>(duration<double, std::milli>(interval_msec));

    emit dataChanged();
}

dataManagement::data dataManagement::getLatestVoltageAndCurrent() {
    if (data_queue.empty()) return {0,0,0,0,0,0, std::chrono::nanoseconds{0}};
    const auto& s = data_queue.back();
    return {s.vAVal, s.cAVal, s.vBVal, s.cBVal, s.vCVal, s.cCVal, s.t}; // 변환 제거
}
...
dataManagement::phasor_data dataManagement::calcPhasor() const {
    ...
    ph.hvMag  = mag(ph.hvReal, ph.hvImag); ph.hvPhaseDeg = ang(ph.hvImag, ph.hvReal);
