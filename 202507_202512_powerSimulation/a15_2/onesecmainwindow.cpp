...
void oneSecMainWindow::onRmsData(dataManagement::measure_data m)
{
    ...
    // --- RMS 누적 ---
    sumFundV2 += m.vFundA * m.vFundA;
    sumFundI2 += m.iFundA * m.iFundA;
    sumHarmV2 += m.vHarmA * m.vHarmA;
    sumHarmI2 += m.iHarmA * m.iHarmA;
    sumTotalV2 += m.vRmsA * m.vRmsA;
    sumTotalI2 += m.iRmsA * m.iRmsA;
    ...
    if (elapsed >= std::chrono::milliseconds(995)) {
        ...
        double osFundVPhase = lastPhasor.vAPhaseDeg;
        double osFundIPhase = lastPhasor.cAPhaseDeg;
