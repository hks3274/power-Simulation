...
QWidget* A3700N_GUI::createPage(const QStringList& listNames)
{
    ...
    for (int i = 1; i < listNames.size(); ++i) {
        ...
        if (baseKey == "analysis" && listNames[i] == "Phasor") {
            ...
            a3700n_phasor* phWidget = new a3700n_phasor(dataMng, phasorVolCheck->isChecked(), phasorCurCheck->isChecked());
            ...
            a3700n_harm* harmWidget = new a3700n_harm(dataMng);
            vLay->addSpacing(-10);
    ...
    for (int p = 0; p < mainStack->count(); ++p) {
        ...
        for (int i = 0; i < stack->count(); ++i) {
            ...
            if (dataKey == "voltage") {
                ...
                if (itemKey == "Volt. Symm.") vals = osl->symmUnbal.voltSymm;
