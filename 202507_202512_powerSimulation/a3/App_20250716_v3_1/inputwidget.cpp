    ...
    : dataMng(dataMng) // 멤버 초기화 리스트,  클래스 내부에 있는 멤버 변수 dataMng를 매개변수 dataMng로 초기화
{
...

    {
        ...
        timeScaleSlideEdit = new slideEdit("시간 비율", dataMng.getTimeScale() , 100, 1);
        vl->addWidget(timeScaleSlideEdit);

        connect(vSlideEdit, &slideEdit::valueChanged, this, &inputWidget::inputChanged);
        connect(timeScaleSlideEdit, &slideEdit::valueChanged, this, &inputWidget::timeScaleChanged);
    }
...
//rps값을 바꿨을 때
void inputWidget::rpsChanged(float newVal)
{
    qDebug() << "Rps: "<< newVal;
    dataMng.setRpsValue(newVal);
}

void inputWidget::timeScaleChanged(int newVal)
{
    qDebug() << "timeScale: "<< newVal;
