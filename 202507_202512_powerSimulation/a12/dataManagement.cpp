...
void dataManagement::captureData()
{
    ...
    //zero-crossing 검출
    //이전의 전압값이 -, 현재 전압값이 + 이면 zero-crossing
    //prevV = 이전의 값, vCurr = 현재의 값
    // ==== Zero Crossing 검출 (전압 - → + 교차) ====
    {
        static bool   first   = true;
        static double prevV   = 0.0;
        static double prevT   = 0.0; // 이전 샘플의 '실제' 시각(초)

        // 이번 샘플의 실제 시각(초) — simulationTime은 duration 누적이므로 OK
        const double tCurr = std::chrono::duration<double>(simulationTime).count();
        const double vCurr = voltageVal;

        if (!first) {
            // 히스테리시스(선택): 너무 0 근처에서 덜컥거림 방지
            constexpr double zc_eps = 1e-9; // 신호가 완전 0일 때 분모 보호용
            // 전압 - -> + 교차 확인
            if (prevV < zc_eps && vCurr >= zc_eps) {
                const double dv = vCurr - prevV;
                const double dt = tCurr - prevT; // 실제 샘플 간격 (interval_msec 기반)

                // 보간: v(t) = prevV + (dv/dt) * (t - prevT) = 0  →  tZC
                // tZC = prevT - prevV * (dt / dv)
                double tZC = tCurr; // fallback
                if (std::fabs(dv) > zc_eps && dt > 0.0) {
                    const double frac = (-prevV) / dv;          // 0~1 근방
                    tZC = prevT + std::clamp(frac, 0.0, 1.0) * dt;
                }

                lastZcSec = tZC; // 기존 필드 유지
            }
        } else {
            first = false;
        }

        // 다음 회차를 위해 '실제' 이전 샘플 정보 저장
        prevV = vCurr;
        prevT = tCurr;
    }
    // ==== /Zero Crossing ====
