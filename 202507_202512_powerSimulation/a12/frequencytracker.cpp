...
void frequencytracker::onRms(dataManagement::measure_data m) {
    ...
    const double dt = (last_t >= 0.0) ? (t_now - last_t) : 0.0;
    last_t = t_now;
    ...
    // 주파수 보정
    const double Kp_freq = 0.3;   // P 게인
    const double Ki_freq = 0.008;   // I 게인
    const double Kd_freq = 0.001;  // D 게인
    ...
    if (dt > 0.0) {
        ...
        freqIntegral = std::clamp(freqIntegral, -3.0, 3.0); //일정량(일정 수치)만큼 증가(감소)하면 더이상 증가(감소) 하지 않는다.
    }
    ...
    if(currentZcTime > 0 && currentZcTime != lastZcTime) {        // 새로운 ZC 검출될 때
        ...
        // 위상 오차를 주파수 보정에 반영
        const double Kp_phase = 0.3;  // P게인
        const double Ki_phase = 0.008;  // I게인
        const double Kd_phase = 0.001; // D게인
        ...
        if (dt > 0.0) {
            ...
            phaseIntegralHz = std::clamp(phaseIntegralHz, -3.0, 3.0);
        }
